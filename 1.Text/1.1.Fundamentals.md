...
A Unicode "code-point" is not a **glyph**. A code-point is an abstract representation of a character in the system. A glyph is an actual visual mark. So, individual glyphs -- for instance, **ligatures** -- are actually defined in individual fonts, not in the Unicode standard. 
...
Once we have an alphabet we can all agree on, we can put text in files and expect it to be readable.
...
And if there are two ways to do things, people will do both. Or either. And so, historically, some systems used LF, some CR, and some, unfortunately (Microsoft we are looking at you), BOTH. 
